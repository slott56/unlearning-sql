...
Note the indirection:

1.  Create a Pydantic ``BaseModel``.

2.  Emit the ``model_json_schema()`` value to create a usable JSON Schema.

3.  Provide that schema to this application to create fake data.
"""
...
class StringGenerator(Iterator[str]):
    ...
    def __next__(self) -> str:
        self.count = self.count + 1
        sz = (
            self.count % (self.maxLength - self.minLength)
            + self.minLength
        )
...
class ArrayGenerator(Iterator[list[Any]]):
    def __init__(self, schema: JSONSchema) -> None:
        self.item_gen: list[Iterator[Any]] = cast(
            list[Iterator[Any]], make_generator(schema["items"])
        )
    ...
    def __next__(self) -> list[Any]:
        self.count = self.count + 1
        data = [next(g) for g in self.item_gen]
        return data
...
class ObjectGenerator(Iterator[dict[str, Any]]):
    def __init__(self, schema: JSONSchema) -> None:
        self.property_map: dict[str, Iterator[Any]] = {
            p: make_generator(schema["properties"][p])
...
def make_generator(schema: JSONSchema) -> Any:
    match schema:
        case {"type": "integer"}:
        ...
        case {"type": "string"}:
            match schema:
                ...
                case (
                    _
                ):  # TODO: https://json-schema.org/understanding-json-schema/reference/string.html#built-in-formats
...
def null_injector(
    property_names: list[str], obj_generator: ObjectGenerator
...
def trash_injector(
    ...
    for p in property_names:
        bad_row = next(obj_generator)
        bad_row[p] = cast(
            AtomicGenerator, obj_generator.property_map[p]
        ).trash()
        yield bad_row
...
def main(
    count=100,
    schema_path=Path("activation_source.schema"),
    output_path=Path("data/activation_source.csv"),
    seed=42,
) -> None:
    random.seed(seed)
    ...
    with output_path.open("w", newline="") as output_file:
        writer = csv.DictWriter(
            output_file, fieldnames=schema["properties"]
        )
        writer.writeheader()
        ...
        if schema["type"] == "object":
            writer.writerows(
                null_injector(schema["properties"], generator)
            )
            bad = len(schema["properties"])
            writer.writerows(
                trash_injector(schema["properties"], generator)
            )
            bad += len(schema["properties"])

    print(
        f"Output: {output_path} has {count} good rows and {bad} bad rows for {schema['title']} schema"
    )
